...
CLASS_DECLARATION( Sentient, Actor, "monster_generic" )
	{
		...
		//{ &EV_Activate, ActivateEvent }, //multiple similar occurences
...
Actor::~Actor()
	...
	//chrissstrahl - code compatibility fix
	//if ( SleepList.ObjectInList( (Actor*)this ) )
	Actor *actorVar = this;

   if ( SleepList.ObjectInList( actorVar ) )
      {
      SleepList.RemoveObject(actorVar);
      }

   if ( ActiveList.ObjectInList(actorVar) )
      {
      ActiveList.RemoveObject(actorVar);
      }
...

   {
	...
	if ( use_pitch_to_enemy )
		...
		if ( currentEnemy )
			...
			dir    = end - pos;
			length = dir.length();
			localAngles = dir.toAngles();

			enemy_dir    = currentEnemy->centroid - pos;
			enemy_angles = enemy_dir.toAngles();

			localAngles[PITCH] = enemy_angles[PITCH];
			localAngles.AngleVectors( &dir );
			end = pos + dir * length;
			}
		}
...
void Actor::SetAttackableByActors
	...
	//qboolean bool;
	qboolean boolVar;
	boolVar = ev->GetBoolean( 1 );
	SetActorFlag( ACTOR_FLAG_ATTACKABLE_BY_ACTORS, boolVar);
	}
...
void Actor::SetImmortal
...

   {
	...
	SetActorFlag( ACTOR_FLAG_IMMORTAL, boolVar );
	}
...
Condition<Actor> Actor::Conditions[] =
   {
		//chrissstrahl - code incompatibility fix
		//{ "default",	returntrue }, //multiple similar occurences fixed
		{ "default",							&returntrue },
...
void Actor::SetTargetable
   ...
   Event *ev
	...
	qboolean boolVar;
	boolVar = ev->GetBoolean( 1 );
	SetActorFlag( ACTOR_FLAG_TARGETABLE, boolVar );
	}
