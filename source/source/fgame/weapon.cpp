...
void Weapon::SetRank( int localOrder, int localRank)
	{
...
void Weapon::SetAmmoAmount( int localAmount, firemode_t mode )
	{
   assert( ( mode >= 0 ) && ( mode < MAX_FIREMODES ) );
...
void Weapon::UseAmmo( int localAmount,   firemode_t mode	)
	{
...
void Weapon::GetMuzzlePosition
   ...
   if ( !this->GetRawTag( "tag_barrel", &barrel_or ) )
      ...
      if ( owner->isSubclassOf( Player ) )
         {
         Player *player = ( Player * )localOwner;
...
void Weapon::Shoot
   ...
   if ( firetype[mode] == FT_PROJECTILE )
      ...
      this->SpecialFireProjectile( pos, 
		...
		meansOfDeath_t localMeansofdeath;
		float knockback;
		...
		localMeansofdeath = GetMeansOfDeath( mode );

      Container<Entity *>victimlist;

		if ( !MeleeAttack( melee_pos, melee_end, damage, owner, localMeansofdeath, 15, -45, 45, knockback, true, &victimlist ) )
   		{
...
void Weapon::AttachToOwner(weaponhand_t localHand)
	{
   AttachGun(localHand);
   ForceIdle();
	}
...
void Weapon::AttachToHolster( weaponhand_t localHand )
   {
   AttachGun( localHand, qtrue );
...
void Weapon::AttachGun( weaponhand_t localHand, qboolean holstering )
	{
	int tag_num;
...
void Weapon::DoneReloading
   ...
   localAmount = ammo_clip_size[mode] - ammo_in_clip[mode];
	
   assert( owner );
	if ( owner && owner->isClient() && !UnlimitedAmmo( mode ) )
		{
      // use up the ammo from the player
      amount_used = owner->UseAmmo( ammo_type[mode], localAmount);
...
void Weapon::WorldHitSpawn( firemode_t  mode, Vector localOrigin, Vector localAngles, float life )
   {
   ...
   ent->setOrigin( localOrigin );
   ent->setAngles( localAngles );
