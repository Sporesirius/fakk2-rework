...
void Gib::Damage
   ...
   direction = ev->GetVector ( 5 );
	...
	if ( blood_spurt_name.length() > 0 )
		{
		localBlood = new Animate;
		localBlood->setModel( blood_spurt_name.c_str() );

		dir[0] = -direction[0];
		dir[1] = -direction[1];
		dir[2] = -direction[2];

		localBlood->angles = dir.toAngles();
		localBlood->setAngles(localBlood->angles );

		localBlood->setOrigin( centroid );
		localBlood->origin.copyTo(localBlood->edict->s.origin2 );
		localBlood->setSolidType( SOLID_NOT );

		localBlood->PostEvent( EV_Remove, 1 );
