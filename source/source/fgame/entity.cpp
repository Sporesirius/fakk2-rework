...
CLASS_DECLARATION( Listener, Entity, NULL )
	{
		...
		//{ &EV_Damage,	DamageEvent }, //multiple occurences fixed
...
void Entity::SetControllerAngles( int num, vec3_t localAngles )
	{
   assert( ( num >= 0 ) && ( num < NUM_BONE_CONTROLLERS ) );
   ...
   VectorCopy( localAngles, edict->s.bone_angles[ num ] );
...
void Entity::SetControllerAngles
	...
	num = ev->GetInteger( 1 );
	localAngles = ev->GetVector( 2 );

	SetControllerAngles( num, localAngles );
   }
