...
void Vehicle::SetDriverAngles( Vector localAngles )
   {
   int i;
   ...
   if ( !driver )
	...
	for( i = 0; i < 3; i++ )
		{
		driver->client->ps.delta_angles[ i ] = ANGLE2SHORT( localAngles[ i ] - driver->client->cmd_angles[ i ] );
		}
   }
	...
	if ( driver )
      ...
      for ( height = 0; height < 100; height += 16 )
         ...
         for ( ang = 0; ang < 360; ang += 30 )
            {
			localAngles[ 1 ] = driver->angles[ 1 ] + ang + 90;
			localAngles.AngleVectors( &forward, NULL, NULL );
