...
inline void SafePtrBase::AddReference( Class *localPtr )

	{
	if ( !localPtr->SafePtrList )
		{
		localPtr->SafePtrList = this;
		LL_Reset( this, nextSafePtr, prevSafePtr );
		}
	else
		{
		LL_Add( localPtr->SafePtrList, this, nextSafePtr, prevSafePtr );
		}
	}

//chrissstrahl - code incompatibility fix
//inline void SafePtrBase::RemoveReference( Class *ptr )
inline void SafePtrBase::RemoveReference( Class *localPtr )
	{
	if ( localPtr->SafePtrList == this )
		{
		if ( localPtr->SafePtrList->nextSafePtr == this )
			{
			localPtr->SafePtrList = NULL;
			}
		else
			{
			localPtr->SafePtrList = nextSafePtr;
