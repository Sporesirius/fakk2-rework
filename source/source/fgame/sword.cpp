...
void Sword::SetWaterDamage( float localAmount, firemode_t mode )
   {
   assert( ( mode >= 0 ) && ( mode < MAX_FIREMODES ) );
   if ( ( mode < 0 ) || ( mode >= MAX_FIREMODES ) )
      {
...
void Sword::SetBasicDamage(float localAmount,firemode_t mode )
...
void Sword::SetWaterRequired(int localAmount,firemode_t mode)
   {
   assert( ( mode >= 0 ) && ( mode < MAX_FIREMODES ) );
   if ( ( mode < 0 ) || ( mode >= MAX_FIREMODES ) )
      {
   ...
   waterRequired[mode] = localAmount;
   }
